  ...
  <div v-if="stats" class="history">
    ...
    <section class="chart">
      <div class="title">
        ...
        <h2 v-if="stats && stats.pomodoros && stats.pomodoros['2018'] && Object.keys(stats.pomodoros['2018']).length">{{ Object.values(stats.pomodoros['2018']).reduce((acc, val) => acc + val) }} Pomodoros in 2018</h2>
        <h2 v-else>0 Pomodoros in 2018</h2>
      </div>
      <Heatmap v-if="stats && stats.pomodoros" :pomodoros="stats.pomodoros['2018']" :start="twentyeighteen" :end="twentynineteen" key="chart"></Heatmap>
      <div v-else class="empty" key="empty">{{ M.history_empty_placeholder }}</div>
    </section>

    <section class="chart">
      <div class="title">
        <h2 v-if="stats && stats.pomodoros && stats.pomodoros['2019'] && Object.keys(stats.pomodoros['2019']).length">{{ Object.values(stats.pomodoros['2019']).reduce((acc, val) => acc + val) }} Pomodoros in 2019</h2>
        <h2 v-else>0 Pomodoros in 2019</h2>
      </div>
      <Heatmap v-if="stats && stats.pomodoros" :pomodoros="stats.pomodoros['2019']" :start="twentynineteen" :end="twentytwenty" key="chart"></Heatmap>
      <div v-else class="empty" key="empty">{{ M.history_empty_placeholder }}</div>
    </section>

    <section class="chart">
      <div class="title">
        <h2 v-if="stats && stats.pomodoros && stats.pomodoros['2020'] && Object.keys(stats.pomodoros['2020']).length">{{ Object.values(stats.pomodoros['2020']).reduce((acc, val) => acc + val) }} Pomodoros in 2020</h2>
        <h2 v-else>0 Pomodoros in 2020</h2>
      </div>
      <Heatmap v-if="stats && stats.pomodoros" :pomodoros="stats.pomodoros['2020']" :start="twentytwenty" :end="twentytwentyone" key="chart"></Heatmap>
      <div v-else class="empty" key="empty">{{ M.history_empty_placeholder }}</div>
    </section>
...
export default {
  data() {
    return {
      ...
      stats: null,
      // historyStart: null,
      twentyeighteen: new Date(2017,11,31,0,0),
      twentynineteen: new Date(2018,11,30,0,0), 
      twentytwenty: new Date(2019,11,28,1,0),
